...
- **What is a cascade?** A rule's effects can trip further rules — the suspension enters some refinement, another rule fires on that. Is the whole cascade one commit, a chain of commits with observable intermediate states, or bounded somehow? The atomicity self-referential folds need generalizes to the entire chain. *Spun out into its own investigation: `investigate_transactions.md` (OQ16–20); this OQ retains act identity, multi-instance commits, and firing-vs-triggering-commit atomicity.*
