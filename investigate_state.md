...
**Mutating a field in place is allowed.** Velle does not force the ledger pattern (below) onto a model that doesn't need history. The goal instead is that when a field is mutated, Velle can tell the product owner the impact — which refinements may change membership, which rules may consequently fire, which `frozen` declarations the write must be provably disjoint from (README §8, "Frozen fields") — and verify that relationships and behaviors stay consistent.
...
- **The target is a literal static path — a hard requirement.** Because `customer.email` is statically known, the whole-spec compiler knows every refinement predicate and derived property that reads `email`, and impact analysis falls out for free: which memberships may change, which entry/exit rules may consequently fire, which `frozen` declarations (README §8, "Frozen fields") the write must be provably disjoint from. A computed or reflective target would destroy this, so no solution for mutation syntax may ever introduce one.
...
Commit-triggered rules already fire exactly once per commit ("Entry and exit are commit-local transitions"), so an ordinary rule needs no guard. A guard earns its place in exactly two situations: **durability** — the firing must survive a crash and be provable afterward — and **cross-tick memory** — a tick rule needs "already handled" as data (the tick law). The canonical form — and the only form; there is no guard sugar ("No guard sugar", below) — is a named refinement whose predicate the rule's own body falsifies:
...
**Re-grounding notes for the README rewrite** (§§11, 12, 14, 15 and every `produces` example): effect creation moves into rule bodies (`X from { ... }`); §12's `for`-query legality re-grounds on guard-derived at-most-one proofs; §12's `compensate X` sugar was defined by desugaring to a `produces`-guarded rule and must either re-desugar to a canonical-guarded rule or fall to the same no-sugar reasoning (since resolved: it fell — retired, `investigate_evidence_policies.md`); §14's one-effect header collapse is gone (a one-effect rule is a one-statement body — "No act-level sugar" reasoning extends).
...
- **Is a rule firing inside its triggering commit, or a subsequent commit of its own?** *Answered in `investigate_transactions.md`: a commit of its own — a firing's effects are a new commit — sharing the triggering commit's transaction by default (`after commit` declares otherwise). Guard soundness's mutation-plus-witness atomicity holds because one firing's body is one commit, never partially applied.*
- **What is a cascade?** A rule's effects can trip further rules — the suspension enters some refinement, another rule fires on that. Is the whole cascade one commit, a chain of commits with observable intermediate states, or bounded somehow? The atomicity self-referential folds need generalizes to the entire chain. *Spun out into its own investigation: `investigate_transactions.md` (OQ16–20); this OQ retains act identity and multi-instance commits.*
- **Where do `then`'s intermediate moments sit?** *Answered in `investigate_transactions.md`: a rule's body is exactly one commit, so `then` orders effects within that single commit — an ordering commitment for compilation, never an observable intermediate state.*
...
- **Re-derive README §13 under commit-local transitions** — resolved for the mutation policies: the clause is dissolved entirely (`stands` named the behavior of silence, `forbidden` became refinement-body `frozen` — README §8, "Frozen fields" — and `compensate` became the evidence-subject entry-rule pattern — README §13; `investigate_evidence_policies.md`). Remaining: what an exit rule may read (captured properties retract at the very moment a `when leaving` rule fires).
