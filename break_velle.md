...
**Correction (`example_predicates.md` #9):** "compose with zero additional mechanism" holds only because the chain length here (nurse → attending, two levels) is small and known when the spec is written, so hand-declaring a shape per level is tolerable. It breaks down the moment the number of levels is itself data (an admin-configured on-call chain of variable depth) — hand-declaring `Escalation1`...`EscalationN` stops being possible. Worked through fully in `example_predicates.md` #9, in two parts: the escalation *process* needs no new mechanism (a self-referential `escalatedFrom: one Escalation?`, same idiom as `LabResult.supersedes` in #6, plus the existing `produces`/event-anchored-`schedule` combination — each hop reacts to the previous one over real time, so no single predicate ever needs to know the chain's depth); cycle validation likewise turns out to be unnecessary, since the same capture-don't-mutate discipline makes a cycle structurally impossible to construct. A recursively-defined value like "the root of this chain" turns out to need no new syntax either — ordinary grammar already permits a derived property to reference itself one hop away (`root = ... parent.root ...`). What's genuinely new is a compiler obligation, not a construct: correctly evaluating a self-referential definition joins the same family as `produces` (safety + liveness) and `requires` (atomicity) — the human states the definition, the compiler is responsible for executing it correctly (`LANGUAGE.md` `## Principles`).
