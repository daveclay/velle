...
- **External input mechanisms (`expose ... using`)** — resolves OQ5, "marking a shape as external input." Velle implements no transport: REST APIs, GraphQL, socket connections to a message bus are traditional-code territory it doesn't compete in; what it owns is the contract between the spec and the shape of the transpiled code. An external mechanism is a *plugin* — traditional code wired into the transpilation step, a citizen of the extension framework (above) — configured and named (`configure DefaultRestAPI using REST { ... }`), then referenced per shape: standalone (`expose CorrectEmail using DefaultRestAPI`) or inline at the shape's own declaration (`expose shape CorrectEmail { ... } using DefaultRestAPI` — a combination of two declarations, not the act-level sugar §12 rejects: nothing is hidden and no positional invariant is spent; the standalone form remains for a second mechanism or an exposure declared elsewhere in the spec). An exposed shape is externally committable via the named mechanism; an unexposed shape enters state only as a rule's effect. The set of `expose` declarations *is* the trust boundary: input-constrained `never` guardrails (§21) compile to the generated surface at each expose site, and reachability sharpens — an act shape with no `expose` and no producing rule is uncommittable, so a rule triggered by it is the unfireable-rule error (§11). v0 exercises the construct with a single builtin mechanism, `MockHarness` (the milestone scope statement, above): "when you transpile this spec, create a function that takes this shape as input," plus a generated `main`-style entry point for manual driving — a runtime-testing spike, deliberately not the plugin design. Deliberately deferred as details of the construct, not open language questions: plugin configuration vocabulary and the plugin API; who-may-commit conditions beyond mechanism (caller identity, ambient policy, rate — OQ20's residue attaches here, at the expose site or its configuration); per-exposure field policy — which fields the committer supplies vs. which are internal (whether an ordinary `initially` field can be marked not-committer-suppliable the way `timestamp` fields inherently are, §5), including supplied-vs-generated `id` at a trust/legacy boundary (a legacy table's primary key, an upstream system's reference — mapping-side mechanics ride with the Mapping item, above); and whether `expose` also names a read surface (querying a shape over the same mechanism) or is strictly input.
