...
- **Every request gets a response** — a transient act no rule answers would be ignored with *no record it ever arrived* (an ordinary act at least sits visibly unprocessed). So its rules' conditions must provably cover every instance in every reachable state — a compile error otherwise, which is the compiler asking the product question "what should happen to this request in this situation?" and refusing to proceed until the spec answers. v0 proves the coarse slice: a rule on the bare act shape, or a complementary pair (`P` / `not P`); per-reason refusals live in one complement rule with a conditional reason value.
