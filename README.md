...
Velle captures discussions between a Product Owner and an Engineer without adding obfuscation and complexity of how that system actually runs on a computer. Reading Velle describes the system - its conceptual models and behaviors - without introducing computer science terms and concepts. The Product Owner and Engineer could read Velle together to understand the system. Velle can generate artifacts useful to Product Owners: design specs and diagrams — class, state-flow, cause-graph, and sequence views derived from the spec (`diagrams.md`). 
...
Interactions are described as shapes with states, not function calls. Instead of a function taking parameters and returning a result or throwing an error, an interaction has an input state and several potential resulting states — success, error, retry, and so on — expressed as ordinary refinements: the errors-are-refinements pattern (assembled end-to-end in `examples/payments/payments.velle`).
...
The CRUD replacement reads off directly: "create" is committing a new instance, "update" is an assignment fired by a rule (`## Assignment (mutation in place)`), "read" is any predicate or derivation evaluated against current state, and "delete" has no primitive at all — a produced fact records something that happened in the world, and deleting the record would make the description lie (`## Exit triggers`); where the business needs reversal, that's a policy expressed as data, not an erasure (`## Open / unresolved`, Reversal).

Because a commit is a discrete moment, pre-state and post-state are both well-defined at it — which is what lets entry into and exit from a refinement mean something precise, with no hidden bookkeeping (`## rule ... when ... on ...`). By default, an act's commit and every commit its consequences produce stand or fall together — one all-or-nothing envelope that a rule opts out of with `after commit` (`## rule`, "Transactions and `after commit`"). An act's commit carries exactly **one statically-shaped closure** — a single instance in the common case, the act plus its declared inline parts when the exposure says the parts arrive with it (`## Relationships`, "Inline part creation"). "Several things arrive together" is not a special commit kind but an ordinary modeling call — a container shape whose single instance is the committed act, with the parts as related shapes — and closure membership is declared, never data-dependent, which is what lets "can these triggers coincide?" be answered from trigger shapes alone (`## Assignment (mutation in place)`, one-writer). Still open: the order-independence and quiescence proofs (OQ16).

### Transient acts (`expose transient`)

By default, committing an act *is* persisting the record — the act is simultaneously a request and a durable business fact (`Payment`, `EmailChange`). But some incoming acts are **inputs to the state, not members of it**: a request whose entire meaning is decided at arrival, whose durable trace should be what the system *did about it*, not the request itself. Marking the exposure `transient` declares that:
...
A transient act's instance exists only within its own commit's transaction: its refinements are evaluated exactly once — at that commit, against the state of that moment — its rules fire, its consequences persist, and at the transaction's close the instance is gone (`evaluation.md`, "Transient acts"). For an ordinary act, a partition like applicable/refused is re-evaluated whenever the partitioning state changes — an act applied long ago can drift into the refused side when the state later flips (the drift hazard, advisory A4; worked exhibit `examples/partition-drift/`). For a transient act that re-evaluation never happens, because there is no later act: the hazard is structurally impossible rather than checked.

The declaration carries two obligations (checks V17–V18):

- **Isolation** — nothing durable or later may depend on the act: no shape may hold a relationship to it (outcome records *copy* the fields they need — which is also the author declaring which slice of the request is durable business data), its name may appear in no expression, and its rules may fire only `on commit` — no `after commit`, no schedules, no `when leaving`. Asynchronous work hangs off a durable intent the handling rule materializes, the intent-before-effect pattern (`## rule`, "Transactions and `after commit`").
- **Every request gets a response** — a transient act nothing answers would be ignored with *no record it ever arrived* (an ordinary act at least sits visibly unprocessed). So the answers must provably cover every instance in every reachable state — a compile error otherwise, which is the compiler asking the product question "what should happen to this request in this situation?" and refusing to proceed until the spec answers. An answer is a rule's condition *or a `never` over the act* — the boundary refusal is a response, and pairing a rule with a complementary `never` is the pure-validation rung: the caller is told, and nothing is kept at all (membership's `AssignTicket`; contrast billing's `ChangeDueDate`, where the refusal is a business record — which to keep is a product decision, made per act). v0 proves the coarse slice: an answer on the bare act shape, or a complementary pair (`P` / `not P`); per-reason refusals live in one complement rule with a conditional reason value.

**Correlating responses to requests.** Because the instance is gone at the transaction's close, there is no system-assigned request id for a caller to point at later. Where a durable outcome must be matched back to the request that caused it — or the same request submitted twice must be applied once — the act carries a **correlation key**: a client-supplied field (`requestKey: text`) whose value every outcome record copies, the idempotency-key pattern real APIs already use (the caller mints one key per logical request, so a resubmission arrives carrying the same key). Everything the pattern needs already exists: the caller supplies the key, outcomes copy it like any other field, and a duplicate-detection guard reads it off the durable outcome (`not exists (BidRecord where requestKey == this.requestKey)`). This is deliberately a documented idiom rather than language machinery (settled OQ26): the key's uniqueness is the client's to own — the language can neither check nor supply it without storing or minting client keys, a layer below the abstraction it occupies. The per-outcome copy and the per-rule guard conjunct are the author's visible decisions about which requests must be idempotent, the same trade as `## Run-once guards`' "No guard sugar"; and two copies of one request racing in flight are simply two transactions, serialized like any others (`evaluation.md`, "The universal transaction"), the later one meeting whatever guard the author wrote.

## 5. Scalars

Property types: `text`, `integer`, `long`, `decimal`, `double`, `boolean`, `Date`, `DateTime`. Their semantics are grounded in Kotlin's corresponding types — Kotlin is the first transpilation target, and a scalar behaves as Kotlin's type unless this document states otherwise: `integer` is `Int`, `long` is `Long`, `double` is `Double`, and `decimal` is `BigDecimal`. The temporal types are `java.time`: `Date` is `LocalDate`, and `DateTime` is `Instant` — a timestamp is a point on the global timeline; a wall-clock moment with a zone attached is domain data, not a scalar. `now` is a `DateTime`; `today` is a `Date` — the current date in the system's one declared time zone (harness-configured in v0 — the milestone scope statement, `## Open / unresolved`).

Examples in this document also use `Money`. It is not a builtin: it stands in for an **author-extended data type** — extending the scalar vocabulary is a planned, post-v0 capability (`## Open / unresolved`).

The builtin **function vocabulary is closed** — a validator can't typecheck calls into an open-ended list, so what ships *is* the list, and growth is ordinary vocabulary expansion rather than redesign. v0 ships: the aggregates and selectors `count`, `sum`, `latest`, `first` (`## Predicate expressions`), `lowercase`, `max`/`min`, the duration constructors `seconds`/`minutes`/`hours`/`days`/`weeks` (written as duration literals — `## Predicate expressions`), and the generator `randomUUID` (`initially` position only). A trailing `?` marks a property optional (`processedOn: Date?`); properties are required by default.

`initially` gives a property a starting value, and it is allowed on any stored property of an unexposed shape, whatever its type, relationships included: `applied: boolean initially false`. (On an *exposed* shape, `initially` and `timestamp` declarations are compile errors — every stored field there is the committer's, and a system-maintained value lives on the unexposed shape a rule materializes; `## Open / unresolved`, "External input".) The field stays stored and assignable (`## Assignment (mutation in place)`); the initializer is evaluated once, at the instance's creation commit — so `submittedOn: Date initially now` records commit time as model data. It is an optional clause on a stored declaration, not a separate property kind — but it is distinct from derivation: `applied: boolean = false` in shape-body position would mean *derived, always false* — unassignable (`## Derived properties`). The two places `initially` can't appear follow from those properties' own rules, not from a restriction on the clause: a derived property has no stored value to initialize, and a `timestamp` field's value is commit metadata, never author-supplied (below). Its refinement-position sibling is `captured`, which evaluates at the entry commit instead of the creation commit (`## Refinements`).
...
`on create` fixes the value at the instance's creation commit; `on update` advances it at every commit that writes a stored field of the instance (creation included, so the field is never absent). The names are the author's — `correctedOn: timestamp on create` is fine. The values are never committer-suppliable (a fact about the commit can't be claimed by the client) and never assignable (`this.updatedAt = now` is a compile error) — which is the point: `updatedAt` maintained by hand would mean every writing rule also assigns it, boilerplate that lies the first time a rule forgets. Contrast `initially now`: also evaluated at the creation commit, but it declares ordinary *stored business data*, assignable ever after (a submission date an admin may later correct); `timestamp on create` declares unassignable bookkeeping fact. Choose by whether the moment is a business datum or a fact about the commit. A declared creation timestamp is also the natural datum to name in a selector's `by` clause (`## Predicate expressions`) — but it is consulted only by name, never implicitly.
...
Direction is not encoded because it would carry no information: inference means both sides always traverse. The traditional parent/child framing conflates three separate things, and these descriptors take exactly one of them — traversal direction (symmetric and free, per the above), **write ownership** (what `one`/`many` declare), and lifecycle dependency ("children die with the parent" — no part of the relationship descriptor; existence-dependency is delete territory (OQ37)). "Invoice has payments" and "payment belongs to invoice" were never two directions of one arrow: only the second is a declaration.
...
### Inline part creation (`with`)

An order committed together with its line items is a different arrival than anything above: the parts *don't exist yet*, so they can't be references — they are values created in the act's commit. Committing the parts first can't serve it (each `OrderLine.order: one Order` needs an order that doesn't exist, and per-part commits are the wrong granularity for "these arrive together"), and `many` never will (references-always, above). The exposure declares it:
...
`with` names a **collection-typed property of the exposed shape** — here the inferred inverse — and that one name determines everything: the part shape (`OrderLine`), the language-populated back-reference (`order`), and the addition to the generated commit function's signature (§22, "External input"): per part, every stored field minus the back-reference, which only the language can populate — a committer-suppliable back-reference would be a claim about an instance that doesn't exist yet. References-always survives trivially: the inferred inverse is derived, not stored, so `orderLines` would never appear in the commit function at all without the `with` — the clause adds a parameter that had no other way to exist, and a declared `many` on an exposed shape keeps meaning references, untouched.

The commit carries the act *plus* its declared parts as **one statically-shaped closure** (§4): membership is declared, never data-dependent, so trigger and one-writer analyses read it from the declaration exactly as they read a single-instance commit. At the closure's commit the entrants are the container and each part — `rule ... when Order` fires once, `rule ... when OrderLine` once per line, all sharing the act's transaction (§20's fan-out precedent); an author who wants once-per-act granularity writes the condition on the container. The order the records physically land in is compilation's business (the topological order over the closure's reference tree, container first) and is never observable: pre-state and post-state are defined at the one commit, and no state where the order exists without its lines ever exists to observe.

Nesting and multiple edges — a flat list for multiple edges; braces when an edge carries its own closure:

```
expose Order with orderLines, shipments
...
Each nested `with` resolves against the part shape the same way. Where no inverse is inferred — two `one` fields sharing a target, the `Transfer` situation above — `with` names the declared view, and the view's recognized form pins the back-reference: `expose Account with outgoing` populates `source`, because `outgoing = (Transfer where source == this)`. An arbitrary-predicate view is a compile error: a closure edge must be the inverse of a declared `one` field (checks catalog, V22).

The rules that keep the construct honest:

- **Closures are trees.** `with` only ever names inverse edges pointing at the enclosing level, so a sibling reference is unspellable — and an in-closure reference targeting an instance created in the same closure is refused: commit it and name it in a later act, or restructure. The tree matches the write side's grain (one hop only, above), and it is what makes the generated input plainly serializable — strictly nested values, no closure-local naming scheme.
- **Inline parts are a bag, not a set.** The set semantics above are about references — two identical handles name the same instance. Two identical inline part values mint two *distinct* instances, each with its own `id`: quantity 1 twice is not quantity 2 once. The post-commit inverse is trivially a set of distinct instances.
- **Creation-only.** The closure exists because its targets can't be referenced. Anything that exists is referenceable, and changing it is already served — an act carrying references plus a rule's assignment (§12) or fan-out (above); nothing update-shaped ever rides the closure, and "create it if it doesn't exist" is a spec decision (a rule, a gate, a `never`), never input structure.
- **Parts never write the container.** A part-triggered rule assigning through the back-reference is N sibling firings writing one field of one instance — a proven write-write conflict, refused totally (checks catalog, V1). The served spellings: derive the aggregate on the container (`total: Money = sum(orderLines.subtotal)` — usually the value was a denormalized derivation all along), or condition on the container for a once-per-act effect.
- **Ordinary instances after the commit.** The closure is a fact about one commit's input, never a property of the instances it creates: once landed, container and parts are updated, frozen, exited, and referenced like anything else.

The generated input structure is a **projection, not a mirror**: the boundary type is the shape minus everything the committer may not claim — no `id`, no `timestamp` fields, no derived properties (§22), and no back-references on inline parts. An out-of-closure relationship generates a typed reference handle (how `id` opacity survives the boundary, §5); a closure edge generates a nested input type, recursively. Everything unrepresentable in the input type needs no runtime refusal, so the runtime surface shrinks to what only runtime can know: the referenced instance exists, no duplicate references, and the input-constrained `never` guardrails at this expose site (§21). Minimum-parts policy needs no closure feature — "an order must have at least one line" is a `never` or gate refinement over committed state (§20, §21); the empty part collection is the absence, as ever. `expose transient` with a closure is refused for now (OQ43).
...
A derived property's formula may reference the same property one hop away through a relationship (self-reference) — e.g. `root: one Foo? = if parent is none then none else (if parent.root is none then parent else parent.root)`. This needs no special syntax; correctly evaluating it is a compiler obligation (`## Principles`), not a language concern — see Predicate expressions, below.
...
- **Captured** — marked with the leading keyword `captured`: evaluated once at the moment the current membership begins, fixed for the duration of that membership, absent before entry, retracted at the close of the exit commit — an exit rule may still read it, as its last reader (`## Exit triggers`) — and re-captured on re-entry. The marker is required because a bare `= expr` in body position is a live derivation — the two kinds must read differently. Capturing `today`/`now` anchors them to the entry moment: `archivedOn` above is the membership's start date, recorded as model data.

`captured` is `initially`'s refinement-position sibling (`## Scalars`): each evaluates its expression once, at the moment its declaring scope begins — `initially` at the creation commit, `captured` at the entry commit. The differences follow from those scopes, not from separate machinery: a creation is forever, so an `initially` field is stored and assignable ever after; a membership can end, so a capture is fixed for its duration and retracted on exit. This is also why `captured` is legal only in a refinement body — only a refinement's predicate provides an entry moment to capture at; a base shape's one such moment is creation, and `initially` is its spelling.
...
**The check is static, whole-spec.** Every write to stored state is a rule assignment to a literal static path (`## Assignment (mutation in place)`), so the compiler knows every writer of every field; the freeze check is the one-writer disjointness question re-aimed — *can this writer's trigger coincide with membership in the freezing refinement?* — fail-closed, reported as one connected diagnostic naming writer and freeze. The fix is the rejection-as-data idiom: partition the act (`ApplicableEdit = LineItemEdit where not invoice is IssuedInvoice` / `RefusedEdit = ...`), hang the writing rule off the applicable subset — now provably disjoint — and let the refusal land as a fact the caller reads back. No commit is ever unwound: immutability needs no rejection machinery, only proofs and partitions. A freeze no rule could ever violate is advisory dead machinery ("serves no writer" — the dead-tolerance shape, `## Self-referential folds and tolerates`).
...
The expression language usable inside `where` — refinement definitions, inline collection filters, and `never` declarations.
...
shape Foo {
    parent: one Foo?
    root: one Foo? = if parent?.root is none then parent else parent?.root
}
```
...
```
latest(Shape for expr by receivedOn)
first(Shape for expr by receivedOn)
latest(Referral where referrer == this by referredOn).referee
```

**Selector ordering is the author's statement — explicit and mandatory.** `latest`/`first` carry a `by` clause naming one or more orderable properties of the element (`latest(payments by receivedOn)`; further comma-separated datums break ties). There is no implicit ordering source: "creation order" is spelled by naming the shape's declared creation timestamp (`by correctedOn`), and a shape with no orderable datum simply cannot be selected over until the author declares one — ordering comes from data, never from minted values (`## Scalars`: `id` is unordered). A selection whose declared datums fail to discriminate the winning position is the model underspecified, and fails loudly rather than silently picking. The sharpest instance is structural: two instances committed in one transaction carry *identical* commit timestamps by definition — a commit is one discrete moment — so discriminating same-transaction siblings always requires an author-carried datum (an act-supplied sequence, a business time), which is the tick law's cousin: ordering, like memory, must be data.

**Duration arithmetic**: `today - 7 days`, `now + 14 days` — an integer literal plus a unit (`seconds`/`minutes`/`hours`/`days`/`weeks`), added to or subtracted from a `Date`/`DateTime`. Semantically, `3 days` is the duration constructor `days(3)` — a function returning a duration value — and `+`/`-` are the duration-accepting operations on `Date`/`DateTime`: sugar the compiler lays over what `java.time` already provides, not a bespoke temporal engine. The step follows the receiver: on a `Date` it is a calendar step (`java.time.Period` semantics — `+ 1 days` is the next calendar date); on a `DateTime` it is exact clock time (`java.time.Duration` — `days(3)` is 72 hours).
...
selectorCall   := ("latest" | "first") "(" collectionExpr
                      "by" Identifier ("," Identifier)* ")"      -- ordering is explicit and mandatory: the author's
                                                                 -- named orderable datums; an undiscriminated winning
                                                                 -- position fails loudly ("by" is contextual, an
...
The **whole-language grammar** — declarations, statements, and value expressions — is `grammar.md`, which incorporates this section's predicate grammar by reference and generalizes its `expr` to full value expressions.
...
**Firing order within a transaction is never specified — and must provably not matter.** Velle states timeless facts, not call sequences: when one commit matches several conditions, the runtime may fire them in any order, or in parallel. Where data flows, the dependency graph orders firings — causality, not policy (`## then`). Where it doesn't, sibling firings must commute, and a spec whose outcome depends on an unstated order is *inconsistent* — a whole-spec compile error naming the rules involved, exactly like one-writer (write-write conflicts *are* one-writer at transaction scope; read-write conflicts and transition interference complete the check). The confluence and quiescence proofs backing this are open (OQ16), as is rejection scope (OQ17).
...
shape Customer {
    ...
    email: text = if exists EmailCorrection for this
                  then latest(EmailCorrection for this by correctedOn).corrected
                  else signupEmail
}
...
Nothing here is new mechanism: `EmailCorrection` is a reified act like any other shape, `latest(... by correctedOn)` names the shape's declared creation timestamp as its ordering datum (`## Scalars` — named, never implicit), and the `exists` check narrows the then-branch so `.corrected` is provably evaluable. Commit means insert, the ledger is append-only, and the stored truth is never touched. The two patterns answer different business realities — "email changed" (in place) versus "email changed and the history matters" (ledger) — and the author picks per field.
...
**What an exit rule may read — the last-reader rule.** At the exit commit the body reads current (post-state) data as any body does, plus exactly one thing more: the leaving refinement's **captured properties**. A capture is stored per-membership memory the runtime demonstrably holds for the membership's duration, and its retraction lands at the *close* of the exit commit, not at the moment the rule fires — so the exit rule is the capture's **last reader**, the one place per-membership memory can be read on its way out, typically to convert what matters into a durable fact before it's gone:
...
rule NotifyArchiver when leaving ArchivedInvoice {
    UnarchiveNotice from {
        invoice: this
        notify: archivedBy        -- the capture's last read, at the exit commit
        noticedOn: now
    }
}
```

This is a re-timing of retraction, not a second evaluation context: base fields read post-state everywhere, a capture has no post-state value to collide with, and no temporal operator exists. Full pre-state access — base fields as they were before the commit, a `before(...)`-style operator — is rejected: reads would be ambiguous for any field the exit-causing commit itself wrote, and the operator would grow into the delta-inspection vocabulary the language excludes (rules react to conditions, never diffs). The leaving refinement's *derived* properties stay unreadable — live computations licensed by a predicate that is now false; there is nothing to re-time. Evaluability is the mirror of the entry-side guardrail (`## Refinements`): beyond ordinary reads, an exit body may reference exactly the captures of the refinement it is leaving. Where the business needs a base field's pre-state ("the balance it recovered from"), the spelling is a capture declared at entry — `captured balanceWhenDelinquent: Money = balance` — the pre-state an exit body may read is the pre-state the author declared they would need; memory is visible data, never ambient context.

The residue is transaction-boundedness: a capture does not survive the exit commit's transaction, so a capture-reading exit rule can never be `after commit` and can never carry a schedule backstop — a compile error in the stranding family ("this rule reads `archivedBy`, which does not survive the transaction boundary it declares"). Evidence remains the only thing that survives *beyond the transaction*: a reaction that must be durable reads the reified act instead of the capture — `notify: latest(ArchiveRequest where invoice == this by requestedOn).requestedBy` (and must say `latest(...)`, not `(ArchiveRequest for this)`: the singular form was licensed by a predicate this instance no longer satisfies, and with re-archival in the model nothing proves at-most-one — `## Predicate expressions`) — or hangs off the evidence itself, the compensation pattern below.
...
```
shape Article {
    viewCount: long tolerates duplication
}
```
...
**Order-dependent folds at a tick: the predecessor recurrence.** A tick-cadence order-dependent fold (a nightly streak sweep) is exposed to reordering that nothing on the rule can discharge: one tick's firings have no defined order (`## Sweeps`), and declared tolerance would be a false statement for a streak. Velle deliberately has no ordering clause on rules — firing order is never language surface — because the order such a fold needs is business data, and it belongs in the data model. The served spelling is a **predecessor recurrence** over ordered history: a derived `previous` (`latest ... by` the ordering datum) plus sanctioned self-reference one hop through it (`## Derived properties`), so each record derives its running value from its predecessor's, and the current total is a selector read. Concretely, the streak:
...
shape Account {
    streak: integer = if exists Payment for this
                      then latest(Payment for this by receivedOn).streakAfter
                      else 0
}
```

No stored field, no guard, no obligation — and more than a fold: the per-record value is readable history ("the streak as of each payment"), and order-sensitivity is structurally impossible, since the order comes from the declared datum rather than from when firings happen to run. Late-arriving records show the recurrence is also the more truthful description: a backdated payment slots into the chain at its datum's position and every successor re-derives, where a stored fold would have folded it last — arrival order — and kept the wrong value with no repair. What the recurrence owes is **well-foundedness** — every chain of definition references must reach a base case in finitely many steps — checked by stratification plus a certificate whitelist (`checks.md` V14): the acyclic part of the definition graph passes free; each static cycle needs a certificate — strict descent on a creation-fixed datum plus a base case, or acyclicity supplied by a `never` invariant (`## never`) — and no certificate is a compile error: the check rejects what it cannot prove rather than warning, and never attempts fixpoint semantics. A tie on the ordering datum where the result depends on it is the model's problem — the author declares a further ordering datum; `id` cannot serve, being deliberately unordered (`## Scalars`). Growing the certificate whitelist is open calibration, one vocabulary shared with the cascade-quiescence proof (OQ16). The mutation form's spelling is commit-cadence, and that is a semantic statement, not a limitation: commits are serialized, so fold order *is* commit order — arrival order — the right description exactly where arrival order is the business order.
...
rule PostTransfer when Transfer {
    LedgerEntry from { account: from, amount: -amount }
...
- **A set that must succeed or fail together is a business object.** Settlement batches, payroll runs: whenever a business says "together," the together-thing has a name — reify it (`## State and commits`' container idiom), and its handling is one record's firing: one transaction, one external act, one `id` to reconcile (intent-before-effect, `## rule`). When the batch is born whole — the parts arrive inline with the act itself — the exposure's closure declaration is this rung's input-side spelling (`## Relationships`, "Inline part creation"); the completeness gate (below) remains the spelling when the members arrive incrementally.
...
- **Latency vocabulary** — `on` expresses the evaluation *source*, not latency *requirements*. Whether "immediate by default, named schedule otherwise" suffices, or deadlines ("within 24h") deserve first-class expression the compiler validates against declared cadences, is undecided. (Moved here from OQ7 at its resolution.)
- **`on commit of <Shape>` narrowing** — "only withdrawals suspend, not fee assessments." Expressible and occasionally meaningful, but it can silently miss entry paths; per flexible-not-restrictive it would be allowed *with* the compiler reporting exactly which entry paths go unobserved. Not yet designed. (Moved here from OQ7 at its resolution.)
- **External input (`expose`)** — resolves OQ5, "marking a shape as external input." Velle implements no transport: REST APIs, GraphQL, socket connections to a message bus are traditional-code territory it doesn't compete in; what it owns is the contract between the spec and the shape of the transpiled code. The construct names no mechanism (it originally carried a `using <Mechanism>` clause over transpilation plugins, dropped when the runtime model settled): transpilation generates one commit function per exposed shape — the thing the engineer's own transport code calls to submit a mutation — and mapping any transport onto those functions is the engineer's code, permanently, not a deferred plugin design. Both declaration forms remain: standalone (`expose CorrectEmail`) or inline at the shape's own declaration (`expose shape CorrectEmail { ... }` — a combination of two declarations, not the act-level sugar §12 rejects: nothing is hidden and no positional invariant is spent; the standalone form remains for an exposure declared elsewhere in the spec). An exposed shape is externally committable; an unexposed shape enters state only as a rule's effect. The set of `expose` declarations *is* the trust boundary: input-constrained `never` guardrails (§21) compile to the generated surface at each expose site, and reachability sharpens — an act shape with no `expose` and no producing rule is uncommittable, so a rule triggered by it is the unfireable-rule error (§11). Who-may-commit conditions (caller identity, ambient policy, rate — OQ20's residue) live in the engineer's wrapper code before the commit call, outside the language. Which fields the committer supplies is read off the declaration forms — there is no field policy to state (resolves OQ31); a closure declaration (`expose ... with`, §6 "Inline part creation") extends the same reading to inline parts, each a nested input value carrying every stored field minus the language-populated back-reference. The generated commit function takes every stored field of the exposed shape as a required parameter: the committer supplies them all, an optional (`?`) field by explicitly supplying the absence, and a convenience default belongs in the engineer's wrapper before the commit call, with the transport mapping it already owns. Derived properties are legal on an exposed shape and are never parameters — nothing is stored, so there is nothing to supply, and the `= expression` form states in the language that the value is calculated (on a transient act, whose one evaluation moment is its own commit, a derived property is also how an arrival-time value is fixed). `initially` and `timestamp` declarations on an exposed shape are compile errors (`checks.md` V21): a system-maintained value is no part of an external submission, so it lives on an unexposed shape — supplied by a materializing rule's `from` block, minted by `initially` at that record's creation commit, or populated as `timestamp` commit metadata — with the transient-act materialization (§4) as the served spelling for a request whose durable record carries such fields. Still deferred as details of the construct, not open language questions: supplied-vs-generated `id` at a trust/legacy boundary (a legacy table's primary key, an upstream system's reference — mapping-side mechanics ride with the Mapping item, above); and whether `expose` also names a read surface (querying a shape) or is strictly input.
- **Reversal** — resolved as a non-issue for the language itself: it's a business-policy choice, expressed via which artifact shapes a human declares. Two candidate idioms exist — the resolution artifact (now exercised as membership's delinquency episodes) and the grace-period suppression window (unexercised; a tracked TODO item) — but no single canonical pattern has been adopted.
...
- **Extensible data types** — author-declared scalar types beyond the builtin list (`## Scalars`). `Money` is the canonical example, demoted from builtin when the scalar list was pinned to Kotlin-grounded types: currency, precision, and literal syntax are exactly the kind of domain judgment an author should declare, not a builtin should assume. What declaring one means — underlying representation, permitted operations, how `sum` and comparisons extend to it — is undesigned. The examples keep using `Money` as a preview of the capability. One citizen of the extension framework, below.
- **The extension framework** — custom data types (above) and custom predicate/value functions are one concept wearing two hats: **registered extensions** — traditional code the developer hands the compiler at execution time through a registration API. Three commitments shape the design. **Registration carries validator-facing metadata**: an extension declares what the checks need to know about it — a custom function's algebraic properties (idempotence, duplication/reordering insensitivity: the `## Self-referential folds` whitelist generalized from a builtin table to a registration contract), a custom type's permitted operations and orderability — so the closed builtin list (`## Scalars`) becomes the *registered* list, and the validator typechecks calls and discharges fold hazards against the registry rather than a hardcoded set. **Extensions are distributable**: workspace-local custom code the compiler locates, or packaged, installable libraries — shared business-domain extensions (a "Velle finance extensions" library carrying `Money`, its operations, its literals) — the same artifact either way. **Extensions are not output**: the transpiled code lives in the developer's workspace and the developer owns it, while extensions live separately, because they change on different axes — a spec edit regenerates output but never touches extensions; an extension upgrade re-runs validation but never rewrites the spec. The resulting architecture has three parts: (1) the language + compiler/validator + environment, which locates extensions in the workspace; (2) the extension framework — types, functions, custom or packaged; (3) the transpile output — the developer-owned executable runtime, *generated executable tests that verify the runtime against the spec*, and opt-in derived diagrams (`diagrams.md`).
...
- **Spec-carried examples (`example`)** — a future construct letting the spec itself declare named example instances, so generated executable specs (`testgen.md`) derive their scenario data from the description instead of demanding human-written givens — the description becomes self-illustrating, collapsing the given/derived split's human tier. Not yet designed; `testgen.md` phase 3.
