...
**Evidence outlives its premise.** A rule fired, its evidence escaped, and then the instance left the refinement: what happens to evidence whose premise has been falsified? Nothing needs declaring — each case is expressed by machinery the language already has (derivation: `investigate_evidence_policies.md`):
