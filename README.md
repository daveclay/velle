...
**Residual gap.** A tick-cadence order-dependent fold (a nightly streak sweep) is exposed to reordering with no honest discharge yet: nothing orders one tick's firings, and declared tolerance is the wrong spelling for a streak. The derivation-side escape is a *predecessor recurrence* over ordered history — self-reference through a derived `previous` plus `latest`, expressible with existing machinery, pending a well-foundedness proof (worked in `open_questions.md`, OQ15; a tie on the ordering datum is the model's problem — the author declares further ordering criteria — not a language gap). Until that lands, commit-cadence is the only fully-served spelling for order-dependent folds.
