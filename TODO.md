...
- [ ] Transaction boundaries, remaining open threads — the boundaries themselves are settled (one-commit default, `after commit`, synced-to-README item above); still open: semantics inside the joined commit (stepwise-vs-endpoint transitions, termination, transitive one-writer), rejection scope and whether commit-refusal is primitive or derivable from reified refusal, and `never` invariants — `open_questions.md` OQ16–17, OQ19–20 (the runtime mechanism — db transaction, orchestrated queue, sagas — is a compiling concern, out of scope there too)
