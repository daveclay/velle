...
- [x] Parser / implementation — **started** (the v0 spec settled first: `grammar.md`, `evaluation.md`, `checks.md`). Kotlin/Gradle project at the repo root: lexer + AST + recursive-descent parser written from `grammar.md`, plus the coarse fail-closed validator from `checks.md` (F1–F4; V1–V8, V10, V13–V16 in v0 form; V11/V12 narrowing and at-most-one proofs still TODO) — `billing.velle` parses and validates clean, and broken-spec fixtures exercise each diagnostic (`src/test/kotlin/velle/`). The `evaluation.md` runtime is also in (`Runtime.kt`/`Evaluator.kt`): in-memory state, transactions with rollback and the never-tripwire, commit-local entry/exit, captures with exit-rule last-read and retraction, synchronous after-commit queue, tick sweeps with previous-clock exit detection — `billing.velle` runs end-to-end under `RuntimeTest`. The MockHarness codegen is in too (`Codegen.kt`, `gradle generate` → `src/main/kotlin/velle/generated/Billing.kt`, golden-tested): per-exposed-shape typed commit functions, tick/clock functions, typed views per shape and refinement (captures included), membership tests, and a runnable `main`. **The v0 spike pipeline is complete: parse → validate → execute → generated typed surface.** Next: write more realistic specs against it and feed what breaks into OQ14–16 calibration
