...
- [ ] Cascade commit boundaries ("transactions") — where atomicity starts and stops when rules trigger other rules; closure vs. chain vs. hybrid, rejection scope for `forbidden`, external effects mid-cascade, atomicity as a declarable per-edge policy — `investigate_transactions.md` OQ16–20 (the runtime mechanism — db transaction, orchestrated queue, sagas — is a compiling concern, out of scope there too)
