...
- [ ] Extension-framework registration API — the developer-facing API for registering custom data types, predicate/value functions, and `expose` mechanisms, with validator-facing metadata (algebraic properties feeding the fold checks, permitted operations, committer-policy hooks) and workspace/packaged-library discovery — README §22, "The extension framework"
- [ ] Given/Then spec-generation tooling — decided conceptually; now placed as part of the transpile output in the extension-framework architecture (generated executable tests that verify the runtime against the spec — README §22, "The extension framework"), no concrete generator designed
- [x] Parser / implementation — **started** (the v0 spec settled first: `grammar.md`, `evaluation.md`, `checks.md`). Kotlin/Gradle multi-module build split along the architecture's compiler/output seam (README §22, "The extension framework"). `compiler/`: lexer + AST + recursive-descent parser written from `grammar.md`, the coarse fail-closed validator from `checks.md` (F1–F4; V1–V8, V10, V13–V16 in v0 form; V11/V12 narrowing and at-most-one proofs still TODO), and the `evaluation.md` runtime (`Runtime.kt`/`Evaluator.kt`): in-memory state, transactions with rollback and the never-tripwire, commit-local entry/exit, captures with exit-rule last-read and retraction, synchronous after-commit queue, tick sweeps with previous-clock exit detection — `billing.velle` parses and validates clean, broken-spec fixtures exercise each diagnostic, and the fixture runs end-to-end under `RuntimeTest`. `output/`: the developer-owned transpiled surface (`gradle generate` → `output/src/main/kotlin/velle/generated/Billing.kt`, golden-tested): per-exposed-shape typed commit functions, tick/clock functions, typed views per shape and refinement (captures included), membership tests, and a runnable `main`. **The v0 spike pipeline is complete: parse → validate → execute → generated typed surface.** Next: write more realistic specs against it and feed what breaks into OQ14–16 calibration
