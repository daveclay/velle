...
- [ ] Transaction boundaries — the commit/transaction distinction (a commit is one mutation; a rule firing's effects are a *new commit*; a transaction is the all-or-nothing envelope containing an act's commit and its consequences' commits), the one-transaction default, the `after commit` trigger preposition (the firing's commit begins a new transaction; boundary inherent at schedule sources, forced at external effects), the transition law (a transition is not data — react within the causing commit's transaction, or reify the obligation as data first), and the two-way clause/apparatus check; syncing requires re-wording README §4 ("together with whatever consequences rules attach to it" — consequences are now commits of their own inside the act's transaction) and re-grounding §18's guard-durability story (the crash window between condition-match and firing only exists at transaction boundaries) — `investigate_transactions.md`
...
- [ ] Transaction boundaries, remaining open threads — the boundaries themselves are settled (one-commit default, `after commit`, synced-to-README item above); still open: semantics inside the joined commit (stepwise-vs-endpoint transitions, termination, transitive one-writer), rejection scope and whether commit-refusal is primitive or derivable from reified refusal, external effects mid-cascade, and boundary-grammar residue (mixed lists, tick granularity, nesting, `tolerates loss`, `never` invariants) — `investigate_transactions.md` OQ16–20 (the runtime mechanism — db transaction, orchestrated queue, sagas — is a compiling concern, out of scope there too)
