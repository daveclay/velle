...
An audit of README §13's mutation-policy triple. Provenance note: these constructs were proposed during design sessions (AI-suggested), not introduced from PO discussions — this file records what problem each exists to solve, what each buys, and whether each survives the language's own precedents (the no-sugar reasoning that retired `produces`, README §18; validation-rejection-as-data, `investigate_transactions.md`). No new open questions are minted here; the analysis lands in existing threads — OQ7 (re-derivation of §13, `compensate`'s desugaring), OQ17 (rejection scope), OQ18 (mid-cascade external effects), OQ20 (is commit-refusal primitive).

**Outcome, recorded up front:** both keywords dissolve, and the policy clause with them. `compensate` is retired in favor of the evidence-subject entry-rule pattern (adopted — README §13, "Compensation is a pattern, not a keyword"). `forbidden` is re-derived as `frozen`, a refinement-body write-gate ("The crack in `forbidden`" onward, below) — its exit-gate semantics fail on monotone states, and the write-gate reformulation needs no commit-rejection at all. `stands` becomes the behavior of writing nothing. The first half of this file records the case *for* the constructs as originally framed; the second half is what re-derivation did to that case.
...
**Decision: retired.** The README documents the evidence-subject entry-rule pattern in §13 ("Compensation is a pattern, not a keyword"), the way §20 documents episodes — accepted, not sugared. Nothing needed salvaging: the pattern is ordinary machinery, and its diagnostics (guard, disarm, one-writer) were already defined.
