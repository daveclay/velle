...
| R12 | 2026-08-25 | bare `when leaving` fires only when the predicate flips — never at deletion. No counterfeit is possible; the residual hazard is omission (a rule meant as "for any reason" silently stops at deletion — worked gap example: the gap probes in `compiler/src/test/kotlin/velle/LeavingRuleDeletionProbeTest.kt`; R16's combined trigger is that author's fix). Implementation lands together with the trigger — dropping the default alone would leave no way to react to a deletion |
| R13 | 2026-08-25 | a `delete` statement whose scope can reach a `when leaving` rule's state (not provably disjoint — the shared prover, R8's posture) is a hard error, not an advisory: the spec is ambiguous about what the rule means for a deleted member, and Velle errors and tells the author rather than compiling an ambiguity. Discharges: a deletion-reaction rule (OQ37 §1), restructure to provable disjointness (the V25/V27 pattern), or an explicit "transition-only" per-rule marker — whose spelling is open (OQ37 §2), designed as one vocabulary with the trigger word |
| R14 | 2026-08-25 | the trigger family is read-symmetric: whichever form a rule uses — predicate-exit, deletion-reaction, or a combined form — the body reads the subject's properties as of the triggering commit, plus the named state's `captured` properties as their last reader. §13's last-reader rule extends to the deletion side unchanged: deletion ends the membership, the capture's retraction lands at that commit's close, so captures CAN be read in a deletion-reaction rule. The two differences the record's non-existence forces — keeping a reference to the subject (V24), delayed firing (R5: no deletion-memory; V28) — may surface only as named compile errors, never as silent behavioral divergence; a combined-form body is checked against the deletion side's constraints. Open: all spellings (trigger word, transition-only marker, combined form), and the bare-shape edge — a record only leaves a bare shape by deletion, so a predicate-exit trigger over a bare shape is inert; whether that is an error, an advisory, or allowed is undecided |
| R15 | 2026-08-25 | write-plus-delete of one instance in one commit is refused **permanently**: "change it and also remove it, at once" is not a business sentence, and the pending-a-use-case hedge is dropped. checks.md V23's implemented refusal — same-body at path granularity, cross-rule at shape granularity — is the settled semantics |
| R16 | 2026-08-25 | the trigger family's spelling: `when leaving X` (predicate exit), `when deleted X` (deletion), `when leaving or deleted X` (either cause) — grammar `"when" ("leaving" ("or" "deleted")? \| "deleted")? condition`. The participle mix is deliberate: both forms elide the same subject-and-copula frame — "when [the record is] leaving" / "when [the record is] deleted" — so `deleted` is an eventive passive, not a past tense, and the voice difference encodes real agency: a record leaves a state by its own data changing, and is deleted by something else. `when deleting` rejected as agency-false (reads as the rule doing the deleting); `when being deleted` rejected as paying words for what the ellipsis already handles. Still open: the transition-only marker's spelling, conditions on the trigger's named state, the bare-shape edge |
...
- **One deleter per instance, per commit** — the one-writer check aimed at existence: two rules deleting the same instance from one commit is the coincidence error; a commit that both writes a field of an instance and deletes it is the same incoherence (there is no business sentence "change it and also remove it, at once") — refused permanently (R15, 2026-08-25).
