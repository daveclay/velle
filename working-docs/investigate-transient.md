...
**Residue:** [OQ26](questions/OQ26-correlation-keys.md) correlation keys · [OQ27](questions/OQ27-erasure.md) erasure/retention (moved out of this doc's former closing section) · TODO.md one-liners: author-supplied refusal message on `never`, provenance/`why` confirmation, V18 growth beyond the complement slice.
...
- **TAX cases, all one idiom** — history acts, evidence acts, and idempotent ingestion each materialize a durable record and copy the payload forward; the durable/transient split becomes an explicit authoring decision. Double-submit confirms the **correlation-key design** (OQ26) as real work (client-supplied keys threaded through outcomes).
