...
**Status:** decided (2026-08-10) and implemented in v0 (2026-08-11) — Design B: a transient act is an input to the state, not a member of it (`expose transient`; README §4, `evaluation.md`, checks V17–V18). Design A is retained below as the documented fallback.
**Residue:** OQ26 correlation keys (settled 2026-08-19 → README §4 "Transient acts": the client-supplied-key idiom stays informal) · [OQ27](questions/OQ27-erasure.md) erasure/retention (moved out of this doc's former closing section) · TODO.md one-liners: author-supplied refusal message on `never`, provenance/`why` confirmation, V18 growth beyond the complement slice.
...
Design B is folded into v0: `expose transient` (grammar.md), removal-at-transaction-close semantics (`evaluation.md`, "Transient acts"), checks V17 (isolation) and V18 (totality, coarse complement slice), Codegen/SpecGen awareness (no accessors or views; Unit-returning givens; outcome-count specs), and README §4's "Transient acts" section with the §12 carve-out. Migrations: billing's `ChangeDueDate`, membership's `AssignTicket`, the exhibit's `TransientEdit` family. Two findings from implementation, both now in the normative docs:

- **Inferred inverses die at the model level** — `Invoice.changeDueDates` was itself a read of transient acts; the inverse collection is simply never synthesized for a transient source.
- **The act's partitions pin to C0** — without this, an act whose *own consequences* flip the partitioning state drifts mid-transaction (a withdrawal that empties the account would be applied *and then refused by itself*). Consequence commits within the transaction never re-partition the act; caught by `TransientActTest`, stated in `evaluation.md`.
...
- **TAX cases, all one idiom** — history acts, evidence acts, and idempotent ingestion each materialize a durable record and copy the payload forward; the durable/transient split becomes an explicit authoring decision. Double-submit confirms the **correlation-key design** (OQ26) as real work (client-supplied keys threaded through outcomes) — since settled: the threaded-key idiom is the whole design (README §4).
...
Moved to its own question: [OQ27 — erasure and retention](questions/OQ27-erasure.md). Design B solves the request-payload slice by construction; the durable-record slice is OQ27's.
