...
- [ ] `transient` acts — a declared-intent marker ("this act must be fully handled at its own commit") that upgrades A4 from advisory to error for the marked shape; nothing is deleted, the contract is static. Design sketch and open sub-questions (spelling, sugar-vs-demand, scope) in `investigate-transient.md`.
