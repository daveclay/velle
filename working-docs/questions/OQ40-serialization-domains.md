...
- **The artifact family ([OQ41](OQ41-compilation-artifacts.md)).** The contention map is one member of a larger set of compilation artifacts for humans — class diagrams, state-flow diagrams, sequence diagrams — homed in OQ41, which also works the deposit and lending-cap worlds into concrete sequence diagrams. The pairing worth keeping straight: a sequence diagram is the *within*-envelope view (causality — OQ16's territory, since sibling commutation is what licenses drawing parallel lanes), while the contention map is the *between*-envelope view (queueing); neither substitutes for the other.
