...
**Status:** open — calibration; the completeness audit ran 2026-08-20 (`audit-sibling-confluence.md`): five check gaps found and fixed (read vocabulary, creation-driven flips, captured values, after/after pairs, `affects()` vocabulary), one genuine order-dependence found in the payments spec and resolved by a spec decision (C1 — release moved to `after commit, Nightly`, held for ratification), the Newman argument written with its residue stated (the cascade induction, disarm sufficiency, N-permutation sampling). The coarse slices are implemented in full. Since OQ15 settled (2026-08-19), this question also carries the V14 certificate-whitelist calibration: definition cycles (V14) and rule cascades (V16) owe the same kind of termination answer, so their certificate vocabulary grows here, once, for both.
...
- **The completeness audit — does "accepted" actually mean "order never matters"?** Executed 2026-08-20; the record is `working-docs/audit-sibling-confluence.md` (the channel matrix, findings F1–F5/C1/T1, the precision work P1–P4, the Newman argument with its residue). Every enumerated divergence channel is now rejected, discharged, or safe by the commit semantics, each verdict pinned by an executable probe (`SiblingConfluenceAuditTest`; the whole-corpus order-permutation sweep in `CommutationSweepTest` re-checks every example world on every build via the runtime's test-only firing-order knob). Each suspect this file previously listed was confirmed and fixed; the payments spec yielded the audit's one genuine order-dependence (C1), resolved spec-side. What the audit leaves open, here: the induction from pairwise-sibling commutation to full-cascade commutation is argued from the recursion structure but not proven; order sampling is identity-versus-reversed, not N-permutations; and the polarity and fresh-instance analyses are deliberately coarse (sums unsigned, `as`-aliased consults unattributed) — each a calibration surface, not a model change.
